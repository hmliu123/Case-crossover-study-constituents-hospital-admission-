...
#Import pollution data for the inclueded 292 cities
b <- read.csv("Data/292city_pol_wei.csv") 
...
meta <- function(dataset,pollution_i){
  ...
  for (lag_indicator_i in lag_indicator) {
   ...
   #Random effects model
    ...
    single_result <- rbind(single_result, res)
...
for (m in subtype){
  ...
  cat(m,"")
  #Import data
  ckd_com <- import(paste0("/Analysis\\All disease\\",
                             m,"_compont.csv"))  

  #Combine health data and pollution data  
  ckd_com <- left_join(ckd_com,a, by = c("city_code","date"))
